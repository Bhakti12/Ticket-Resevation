...
export default class adminController extends globalSuccessHandler {
  ...
  constructor(
    @inject(TYPES.AuthenticationService) authService: IAuthenticationService,
  ...
  async changeUserStatus(req: express.Request, res: express.Response) {
    try {
      ...
      const stausChange = await this._adminService.chnageUserStatus(
        userId,
        status
      );
  ...
  async getAllEvents(req: express.Request, res: express.Response) {
    try {
      const events = await this._adminService.getAllEvents();
      this.sendJsonResponse(res, "events", { length: 1 }, events);
    } catch (err) {
      this.sendErrorResponse(req, res, err);
    }
  }
